...
<div class="container mb-3" style="margin-top:70px;">
    ...
    <?php if (count($data['data']) == 0) : ?>
        <p style="font-size:small;">Tidak ada data ditemukan. <a href="<?= base_url(); ?>/transaksi/page/<?= $data['last']; ?>">Kembali</a></p>
    <?php else : ?>
    ...
    <nav class="mt-2" aria-label="...">
        <ul class="pagination pagination-sm">
            <li class="page-item <?= $data['page'] == 1 ? 'disabled' : ''; ?>">
                <a href="<?= base_url(); ?>/transaksi/page/<?= $data['page'] - 1; ?>" class="page-link">Prev</a>
            </li>
            <?php foreach ($data['showpage'] as $i) : ?>
                <li class="page-item <?= $i == $data['page'] ? 'active' : ''; ?>"><a class="page-link" href="<?= base_url(); ?>/transaksi/page/<?= $i; ?>"><?= $i; ?></a></li>
            <?php endforeach; ?>
            <li class="page-item <?= ($i > $data['last']) ? 'disabled' : ''; ?>">
                <a href="<?= base_url(); ?>/transaksi/page/<?= $data['page'] + 1; ?>" class="page-link">Next</a>
            </li>
        </ul>
    </nav>
</div>
