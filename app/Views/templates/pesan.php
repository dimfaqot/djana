...
<body style="margin-bottom:100px;">
            ...
            <div class="container">
                ...
                <ul class="navbar-nav me-auto">
                    ...
                    <?php foreach (menu() as $i) : ?>
                        <li class="nav-item d-none d-md-block">
                            <a class="nav-link <?= ($judul == $i['menu'] ? 'active' : ''); ?>" aria-current="page" href="<?= base_url(); ?>/<?= strtolower(str_replace(" ", "", $i['menu'])); ?>"><?= $i['menu']; ?> <?= ($i['menu'] == 'Transaksi' ? '<span class="text-danger">(' . counttransaksi() . ')</span>' : ''); ?><?= ($i['menu'] == 'Tugas' ? '<span class="text-danger">(' . counttugas() . ')</span>' : ''); ?></a>
                        </li>
                    <?php endforeach; ?>
                </ul>
    ...
    <script>
        ...
        $(document).on('keyup', '.carinotif', function(e) {
            e.preventDefault();
            let value = $(this).val().toLowerCase();

            $('.listcarinotif').filter(function() {
                $(this).toggle($(this).data('lists').toLowerCase().indexOf(value) > -1);
            });
        });
        $(document).on('keyup', '.cariinv', function(e) {
            e.preventDefault();
            let value = $(this).val().toLowerCase();

            $('.bodyinv tr').filter(function() {
                $(this).toggle($(this).data('val').toLowerCase().indexOf(value) > -1);
            });
        });
        ...
        $(document).on('keyup', '.cariproduk', function(e) {
            e.preventDefault();
            let val = $(this).val();
            let display = $(this).data('display');

            post('produk/cariproduk', {
                    val
                })
                .then(res => {
                    if (res.status == '200') {
                        ...
                        for (let i = 0; i < res.data.length; i++) {
                            html += '<a href="<?= base_url(); ?>/produk/search/' + res.data[i].produk + '" data-display="' + display + '" type="submit" class="list-group-item list-group-item-action submitcariproduk">' + res.data[i].produk + '</a>';
                        }
                        $('.body' + display).html(html);
                    }
                })

        });

        $(document).on('click', '.btneditkeranjang', function(e) {
            e.preventDefault();
            let id = $(this).data('id');
            let order = $(this).data('order');

            let val = parseInt($('.submitjumlah' + id).val());

            if (order == 'tambah') {
                val = val + 1;
                $('.submitjumlah' + id).val(val);
            }
            if (order == 'kurang') {
                val = val - 1;
                $('.submitjumlah' + id).val(val);
            }
            $('.pilih' + id).attr('data-qty', val);
        ...
        $(document).on('blur', '.editkeranjang', function(e) {
            e.preventDefault();
            let id = $(this).data('id');
            let order = $(this).data('order');

            let val = $('.submitjumlah' + id).val();
            ...
            post('keranjang/edit', {
                ...
                .then(res => {
                    if (res.status == '200' && order == 'editkeranjang') {
                        if (val == "" || val <= 0) {
                            location.reload();
                        }
                    }
        ...
        $(document).on('change', '.pilih', function(e) {
            ...
            $('input[name="produk"]:checked').each(function() {
                total = (total + $(this).data('harga')) * $(this).data('qty');
        ...
        $(document).on('click', '.transaksi', function(e) {
            e.preventDefault();
            let inventaris = $('input[name="inventaris"]:checked').val();
            if (inventaris === undefined) {
                inventaris = 'off';
            }
            ...
            if (id.length == 0) {
                $('.gagal').show();
                $('.pesangagal').text('Tidak ada produk yang dipilih!');
                return false;
            }
            post('transaksi', {
                    id,
        ...
        $(document).on('keyup', '.caritransaksi', function(e) {
            e.preventDefault();
            let val = $(this).val();
            let display = $(this).data('display');

            post('transaksi/caritransaksi', {
                    val
                })
                .then(res => {
                    if (res.status == '200') {
                        let html = '';
                        for (let i = 0; i < res.data.length; i++) {
                            html += '<a href="<?= base_url(); ?>/transaksi/search/' + res.data[i].produk + '" data-display="' + display + '" type="submit" class="list-group-item list-group-item-action submitcariproduk">' + res.data[i].produk + '</a>';
                        }
                        $('.body' + display).html(html);
                    }
                })
        ...
        $(document).on('keyup', '.caritugas', function(e) {
            e.preventDefault();
            let val = $(this).val();
            let display = $(this).data('display');

            post('tugas/caritugas', {
                    val
                })
                .then(res => {
                    if (res.status == '200') {
                        let html = '';
                        for (let i = 0; i < res.data.length; i++) {
                            html += '<a href="<?= base_url(); ?>/tugas/search/' + res.data[i] + '" data-display="' + display + '" type="submit" class="list-group-item list-group-item-action submitcariproduk">' + res.data[i] + '</a>';
                        }
                        $('.body' + display).html(html);
                    }
                })
            ...
            post('user/cariuser', {
                ...
                .then(res => {
                    if (res.status == '200') {
                        let html = '';
                        for (let i = 0; i < res.data.length; i++) {
                            html += '<form style="width:70%;" method="post" action="' + baseUrl + '/user/search">';
                            html += '<input type="hidden" name="id" value="' + res.data[i].id + '">';
                            html += '<button type="submit" class="list-group-item list-group-item-action">' + res.data[i].nama + '</button>';
                            html += '</form>';
                        }
                        $('.bodycariuser').html(html);
                    }
                })
